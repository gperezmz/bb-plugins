...
A sidebar thread list for [bb](https://getbb.app). It keeps everything bb's own list does, and shows at a glance each thread's state, the harness that runs it, and what its child threads are doing, without the child threads taking over the list. A **Needs you** section at the top of the list holds every family with a thread only you can move forward.
...
- [What "Needs you" means](../../docs/explanation/thread-glance-attention.md)
