...
A sidebar thread list for [bb](https://getbb.app). It keeps everything bb's own list does, and shows at a glance each thread's state, the harness that runs it, and what its child threads are doing, without the child threads taking over the list. A **Needs attention** filter keeps only what you alone can move forward.
