...
| OpenAI-compatible inference | Registers the `gateway` and `local` services; sends its settings to the host entry | none | Answers bb's helper completions on the server machine |
...
| OpenAI-compatible inference | `<data dir>/plugins/openai-inference/host-data/endpoints.json`, readable only by bb's user | A copy of its settings, keys included, for the host entry |
